...
import {
  signInWithGoogle, signOutFromFirebase, getCurrentFirebaseUser,
  getStoredFirebaseSession, pullFromCloud, pushAllToCloud,
...
/* ==========================================================================
   Section : Compte & Synchronisation cloud (Firebase)
   ========================================================================== */
async function renderCloudSyncSection(container) {
  const liveUser = getCurrentFirebaseUser();
  const storedSession = await getStoredFirebaseSession();
  const user = liveUser || storedSession; // Affiche session mêmémoire si SDK pas encore chargé
  const lastSync = await getSetting('lastCloudSync', null);
  ...
  if (user) {
    ...
    container.innerHTML = `
      <div class="panel" style="margin-bottom:16px;">
        <div class="panel-header"><h3>Compte &amp; Synchronisation</h3></div>
        <div class="stat-row">
          <span class="stat-row-label">Compte Google</span>
          <span style="font-size:13px;">${escapeHtml(user.email || user.displayName || user.uid)}</span>
        </div>
        <div class="stat-row" style="margin-top:8px;">
          <span class="stat-row-label">Statut</span>
          <span class="badge badge-pos">Connecté — sync active</span>
        </div>
        <div class="stat-row" style="margin-top:8px;">
          <span class="stat-row-label">Cloud</span>
          <span style="font-size:12px;color:var(--text-muted);">${escapeHtml(lastSyncText)}</span>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:16px;">
          <button type="button" class="btn btn-ghost" id="cloud-sync-now-btn">Synchroniser maintenant</button>
          <button type="button" class="btn btn-ghost" id="cloud-signout-btn" style="color:var(--danger);">Se déconnecter</button>
        </div>
        <p style="font-size:11.5px;color:var(--text-muted);margin-top:12px;">Vos données sont automatiquement sauvegardées sur votre compte Google via Firebase. Se connecter depuis un autre appareil restaurera toutes vos données.</p>
      </div>`;

    container.querySelector('#cloud-sync-now-btn').addEventListener('click', async () => {
      const btn = container.querySelector('#cloud-sync-now-btn');
      btn.textContent = 'Synchronisation...';
      btn.disabled = true;
      const result = await pushAllToCloud();
      btn.textContent = 'Synchroniser maintenant';
      btn.disabled = false;
      showToast(result.success ? 'Données synchronisées avec le cloud.' : `Échec : ${result.reason}`);
      if (result.success) renderCloudSyncSection(container); // rafraîchi lastSync
    });

    container.querySelector('#cloud-signout-btn').addEventListener('click', async () => {
      const ok = await confirmDialog('Se déconnecter du compte Google ? La synchronisation cloud sera désactivée. Vos données locales restent intactes.');
      if (!ok) return;
      await signOutFromFirebase();
      showToast('Déconnecté du compte cloud.');
      renderCloudSyncSection(container);
    });
    ...
    container.innerHTML = `
      <div class="panel" style="margin-bottom:16px;">
        <div class="panel-header"><h3>Compte &amp; Synchronisation</h3></div>
        <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;">Connectez-vous avec Google pour sauvegarder vos données dans le cloud et les retrouver sur n’importe quel appareil.</p>
        <button type="button" class="btn btn-primary btn-block" id="cloud-signin-btn">
          <svg viewBox="0 0 24 24" width="18" height="18" style="vertical-align:middle;margin-right:8px;"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
          Se connecter avec Google
        </button>
        <p style="font-size:11.5px;color:var(--text-muted);margin-top:10px;">Mode local actif — vos données restent privées sur cet appareil sans connexion. La connexion Google est optionnelle.</p>
      </div>`;

    container.querySelector('#cloud-signin-btn').addEventListener('click', async () => {
      const btn = container.querySelector('#cloud-signin-btn');
      btn.textContent = 'Connexion...';
      btn.disabled = true;
      try {
        const user = await signInWithGoogle();
        if (!user) {
          btn.innerHTML = `<svg viewBox="0 0 24 24" width="18" height="18" style="vertical-align:middle;margin-right:8px;"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>Se connecter avec Google`;
          btn.disabled = false;
          return;
        }

        // Vérifier si l'utilisateur a déjà des données cloud
        btn.textContent = 'Vérification...';
        const pullResult = await pullFromCloud();

        // Si le pull a réussi ET qu'il y avait des données locales AVANT connexion, proposer la fusion
        const localData = await import('../db.js').then((m) => m.exportAllData());
        const hasLocalData = Object.values(localData.stores).some((rows) => rows?.length > 0);

        if (hasLocalData && pullResult.success) {
          // Les données cloud ont été fusées dans IndexedDB par pullFromCloud()
          // Proposer maintenant de pousser également les données locales vers cloud
          const migrate = await confirmDialog('Vous avez des données locales existantes. Voulez-vous les importer dans votre compte cloud ?');
          if (migrate) {
            btn.textContent = 'Import en cours...';
            await pushAllToCloud();
            showToast('Données locales migrées vers le cloud.');
          }
        } else if (!pullResult.success && hasLocalData) {
          // Premier login — pas de données cloud, proposer d'envoyer les locales
          const migrate = await confirmDialog('Voulez-vous importer vos données locales dans votre compte Google ?');
          if (migrate) {
            btn.textContent = 'Import en cours...';
            await pushAllToCloud();
        ...
        showToast(`Connecté en tant que ${user.email || user.displayName}.`);
        renderCloudSyncSection(container);
        notifyDataChanged('all');
      } catch (err) {
        showToast(err.message || 'Erreur de connexion Google.');
        btn.innerHTML = `<svg viewBox="0 0 24 24" width="18" height="18" style="vertical-align:middle;margin-right:8px;"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>Se connecter avec Google`;
        btn.disabled = false;
      }
    });
...
export async function renderSettings() {
  ...
  if (!container) return;
  container.innerHTML = '<div id="settings-cloud"></div><div id="settings-profile"></div><div id="settings-security"></div><div id="settings-notifications"></div><div id="settings-install"></div><div id="settings-update"></div><div id="settings-dashboard"></div><div id="settings-features"></div><div id="settings-currency"></div><div id="settings-backup"></div><div id="settings-credit"></div>';
  await renderCloudSyncSection(document.getElementById('settings-cloud'));
  ...
  document.getElementById('settings-credit').innerHTML =
    '<p style="text-align:center;font-size:11px;color:var(--text-faint);margin:20px 0 4px;">Par Adtcheko 5T/ &middot; <a href="mailto:ronywest01@gmail.com" style="color:inherit;">Contribuer / contact</a></p>';
}
